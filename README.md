...
tab rows say.** Real per-school term dates already vary (that's the whole
point of the Terms tab) and are used everywhere else that needs them -
gating cost, gating weekly revenue, the P&L archive. A single "termly"
headline across Evening/Day/Combined needs one window to sum over, so it
deliberately doesn't try to resolve whose real term that is. See
`termBounds()`/`TERM_DEFS` in app.js if those calendar boundaries ever
need adjusting.
...
Lives on the **Actual tab**, below the this-week cards: pick a from and to
